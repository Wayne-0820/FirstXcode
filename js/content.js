...
const CONTENT = {
  ...
  //  宗門：開宗立派，廣納弟子。
  //
  //  ★ 弟子跟道侶、法寶一樣是「永久」的 —— 突破帶不走。
  //    但它跟所有其他系統都不同的地方是：**弟子會自己隨時間變強**。
  //    招募之後不用你再做任何事，弟子日夜修煉、道行自己往上爬，
  //    加成跟著長。這是唯一「掛著就會進步」的系統 ——
  //    正好是放置遊戲該有的味道，也是「飛昇後沒事做」的解答之一。
  //
  //  道行（弟子等級）：招募時 Lv1，之後每累積 hoursPerLevel 小時的修煉
  //    升一級，最高 maxLevel。這個「累積時間」不吃離線上限 ——
  //    離線收益最多算 8 小時，但弟子的道行放多久就長多久（見 state.js
  //    的 advanceSect）。放一週回來，弟子是真的長大了。
  //
  //  加成 = base × (1 + (道行 − 1) × perLevel)，同種效果全部相加，
  //    收斂成「1 + 總和」的倍率（跟武學、裝備同一套加法，不會失控）。
  //
  //  宗門等級 = 已招募的弟子數。它決定弟子逐步解鎖（requireRank）——
  //    一次把八個弟子全攤出來只會讓人眼花；也決定畫面上的宗門稱號。
  //
  //  effect 跟法寶／道侶／武學共用：power / allRate / drop / click / insightGain
  //  cost        : 用悟性招募
  //  requireRank : 宗門等級（已收弟子數）到這個數才會出現
  //  base        : Lv1 的基礎加成比例，0.15 = +15%
  //  quote       : 招募時說的話
  // ----------------------------------------------------------
  sect: {
    name: "宗門",
    desc: "開宗立派，弟子日夜修煉，道行與你共增。",
    maxLevel: 10,        // 道行最高幾級
    hoursPerLevel: 2,    // 每累積幾小時的修煉升一級道行
    perLevel: 0.25,      // 每級道行 +25% 的基礎加成
    // 宗門稱號：依已招募的弟子數，index = 收了幾個弟子（夾到最後一個）
    ranks: ["草創", "小宗", "名門", "洞天福地", "一方仙宗", "天下聖地"],
  },

  disciples: [
    { id: "d_qin",   name: "大師兄·秦淵",   icon: "🧑‍🎓", effect: "allRate", base: 0.15, cost: 8,   requireRank: 0,
      quote: "「師父閉關去了，宗門先交給我。」" },
    { id: "d_lin",   name: "劍童·林小滿",   icon: "🧒",   effect: "power",   base: 0.20, cost: 10,  requireRank: 0,
      quote: "「我要成為天下第一劍！」" },
    { id: "d_he",    name: "藥童·青禾",     icon: "🧑‍🌾", effect: "drop",    base: 0.20, cost: 14,  requireRank: 1,
      quote: "「採藥三年，才認得半座山。」" },
    { id: "d_mo",    name: "沉默師弟·墨白", icon: "🧑‍🦱", effect: "allRate", base: 0.25, cost: 22,  requireRank: 2,
      quote: "「……（他從不多話）」" },
    { id: "d_yan",   name: "護法·燕無雙",   icon: "🧝",   effect: "power",   base: 0.40, cost: 35,  requireRank: 3,
      quote: "「宗門在，我在。」" },
    { id: "d_su",    name: "琴師·蘇冷月",   icon: "🧝‍♀️", effect: "click",   base: 0.60, cost: 45,  requireRank: 4,
      quote: "「一曲清心，坐忘塵勞。」" },
    { id: "d_zhang", name: "首座·張真人",   icon: "🧙",   effect: "allRate", base: 0.50, cost: 70,  requireRank: 5,
      quote: "「修行如逆水行舟，弟子當自強。」" },
    { id: "d_gu",    name: "太上長老·孤鴻", icon: "🧙‍♂️", effect: "insightGain", base: 0.20, cost: 120, requireRank: 6,
      quote: "「我送走過三個紀元的飛昇者。」" },
  ],
